...
const loadCarousel = async () => {
  try {
    const result = await Request({
      url: '/api/carousel/getCarouselByCategory',
      params: {
...
const loadVideosByCategory = async (isLoadMore = false) => {
  ...
  loading.value = true;
  try {
    console.log('Loading videos by category:', categoryId.value)
    const result = await Request({
...
watch(() => route.params, (newVal) => {
  if (newVal.categoryId) {
    ...
    categoryName.value = route.query.name || '分类'
    // 重置分页状态
    pageNo.value = 1
    hasMore.value = true
    videos.value = []
